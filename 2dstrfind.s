...
ifContainHor:
        ...
        addi $sp, $sp, -4
        sw   $ra, 0($sp)
        jal  containHor                  # $v0 = containHor(grid + grid_idx, word)
        lw   $ra, 0($sp)
        addi $sp, $sp, 4
...
#		diagonally
        ...
        beq  $v0, $zero, ifContainVer    # if (!containHor) check vertically
...
#		else checkDiagonal
        ...
        addi $sp, $sp, -4
        sw   $ra, 0($sp)
        jal  containVer                  # $v0 = containVer(grid + grid_idx, word)
        lw   $ra, 0($sp)
        addi $sp, $sp, 4
        ...
        beq  $v0, $zero, ifContainDiag   # if (!containVer) check diagonally
        addi $s4, $s4, 1                 # words_found += 1
        li   $v0, 1                      # set syscall for print int
        move $a0, $t4                    # print_int($t4) ; t4 = row_num
        syscall
...
#		else increment while loop
        ...
        addi $sp, $sp, -4
        sw   $ra, 0($sp)
        jal  containDiag                 # $v0 = containVer(grid + grid_idx, word)
        lw   $ra, 0($sp)
        addi $sp, $sp, 4
